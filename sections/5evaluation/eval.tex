...
\begin{figure}[ht]
  ...
  \caption[Predicted epoch reward during training of agent in world model]{Plot}
  \label{fig:eval:world-model-pred-reward}
\end{figure}

Figure \ref{fig:eval:world-model-pred-reward} shows the reward (decrease in estimated runtime) for each graph as predicted by the world model during training. 
