...
Modifying the softmax activation function in this way is the same as in the knowledge distillation process in which so-called ``dark knowledge'' is transferred from a large teacher model, or ensemble model, to a smaller model which acts as a student model. In both the context of knowledge distillation and training a controller actor inside the world model, a high temperature will generate a softer pseudo-probability distribution for the values of $\pi$ in the GMM. Additionally, using soft targets will provide a greater amount of information for the model to be learn by forcing the model to learn more aggressive policies, outputting more stochastic predictions which is beneficial to explore the environment state-action space.

\subsection{Action Controller}

\subsection{Training}

\subsubsection{Model-Free Agent}

\subsubsection{Model-Based Agent}
- Random agent to train MDN-RNN and GNN in tandem

- Train controller agent inside the world model (MDN-RNN) using PPO based on state, reward, terminal predictions from the world model